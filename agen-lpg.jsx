  ...
  return (
    <div style={{ ...theme, fontFamily: "'DM Sans', 'Segoe UI', sans-serif", background: "var(--bg)", minHeight: "100vh", display: "flex", color: "var(--text)", transition: "background 0.4s, color 0.4s" }}>
      ...
      {role && (<>
          ...
          <div style={{ padding: 24 }}>
            ...
            {page === "dashboard" && (<div style={{ animation: "fadeIn .3s" }}>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(260px,1fr))", gap: 16, marginBottom: 24 }}>
                ...
                <div style={{ background: "var(--card)", borderRadius: 16, border: "1px solid var(--border)", padding: 20 }}>
                  ...
                  {products.slice().sort((a, b) => { const qa = del.filter(o => o.pid === a.id).reduce((s, o) => s + o.qty, 0); const qb = del.filter(o => o.pid === b.id).reduce((s, o) => s + o.qty, 0); return qb - qa; }).slice(0, 3).map((p, i) => {
                    const qty = del.filter(o => o.pid === p.id).reduce((s, o) => s + o.qty, 0); const maxQty = del.reduce((s, o) => s + o.qty, 0); return (
            ...
            {page === "pangkalan" && (<div style={{ animation: "fadeIn .3s" }}>
              ...
              {pkTab === "perf" && <div style={{ background: "var(--card)", borderRadius: 16, border: "1px solid var(--border)", overflow: "hidden" }}><div style={{ overflowX: "auto" }}>
                <table style={{ width: "100%", borderCollapse: "collapse" }}><thead><tr>{["Pangkalan", "Status", "Rating", "Kuota", "Total Distribusi", "Stok", "Bergabung"].map(h => <th key={h} style={{ textAlign: "left", padding: "10px 16px", fontSize: 10, fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.06em", color: "var(--text-dim)", background: "var(--card2)", borderBottom: "1px solid var(--border)" }}>{h}</th>)}</tr></thead>
                  <tbody>{pangkalan.slice().sort((a, b) => b.td - a.td).map(pk => {
                    const qp = pct(pk.qu, pk.qm); return (
            ...
            {page === "stock" && (<div style={{ animation: "fadeIn .3s" }}>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(280px,1fr))", gap: 16 }}>
                {products.map(p => {
                  const pr = Math.min((p.stock / (p.min * 5)) * 100, 100); const low = p.stock <= p.min; const totalSold = del.filter(o => o.pid === p.id).reduce((s, o) => s + o.qty, 0); return (
              ...
              <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(380px,1fr))", gap: 16, marginBottom: 20 }}>
                ...
                <div style={{ background: "var(--card)", borderRadius: 16, border: "1px solid var(--border)", padding: 22 }}>
                  ...
                  <div style={{ display: "flex", alignItems: "flex-end", gap: 6, height: 70 }}>
                    {fulfillment.weekDist.map((v, i) => {
                      const max = Math.max(...fulfillment.weekDist, 1); return (
              ...
              <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(340px,1fr))", gap: 16, marginBottom: 20 }}>
                ...
                <div style={{ background: "var(--card)", borderRadius: 16, border: "1px solid var(--border)", padding: 22 }}>
                  ...
                  {anomalies.length === 0 ? (
                    ...
                    <div>{anomalies.map((a, i) => {
                      const d = new Date(); d.setDate(d.getDate() - (29 - a.day)); return (
                  ...
                  <div style={{ display: "flex", alignItems: "flex-end", gap: 4, height: 80 }}>
                    {predOrders.map((v, i) => {
                      const max = Math.max(...predOrders, 1); const d = new Date(); d.setDate(d.getDate() + i + 1); return (
              ...
              <div style={{ background: "var(--card)", borderRadius: 16, border: "1px solid var(--border)", overflow: "hidden" }}>
                ...
                <div style={{ overflowX: "auto" }}><table style={{ width: "100%", borderCollapse: "collapse" }}><thead><tr>{["Produk", "Terjual", "Pendapatan", "Keuntungan", "Margin"].map(h => <th key={h} style={{ textAlign: "left", padding: "10px 18px", fontSize: 10, fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.06em", color: "var(--text-dim)", background: "var(--card2)", borderBottom: "1px solid var(--border)" }}>{h}</th>)}</tr></thead>
                  <tbody>{products.map(p => {
                    const s = del.filter(o => o.pid === p.id); const r = s.reduce((a, o) => a + o.total, 0); const pr = s.reduce((a, o) => a + o.profit, 0); const q = s.reduce((a, o) => a + o.qty, 0); return (
        ...
        {/* ═══ ROLE: ROUTES (Driver) ═══ */}
        {role && page === "routes" && (() => {
          const RC = ROLES.driver?.color || "#3b82f6"; return (
            <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.5)", backdropFilter: "blur(6px)", zIndex: 50, display: "flex", justifyContent: "center", alignItems: "center", padding: 20, animation: "fadeIn .2s" }} onClick={() => setPage("dashboard")}>
          ...
          <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.5)", backdropFilter: "blur(6px)", zIndex: 50, display: "flex", justifyContent: "center", alignItems: "center", padding: 20, animation: "fadeIn .2s" }} onClick={() => setPage("dashboard")}>
            <div style={{ background: "var(--card)", borderRadius: 20, padding: 28, width: "100%", maxWidth: 700, maxHeight: "90vh", overflowY: "auto", border: "1px solid var(--border)", animation: "slideUp .3s" }} onClick={e => e.stopPropagation()}>
              ...
              {[...new Set(pangkalan.map(p => p.kec))].map(kec => {
                const pks = pangkalan.filter(p => p.kec === kec); const tot = pks.reduce((s, p) => s + p.td, 0); return (
                  ...
                  <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(300px,1fr))", gap: 16 }}>
                    ...
                    <div style={{ background: "var(--card2)", borderRadius: 14, padding: 18 }}>
                      ...
                      {Object.entries(byMethod).sort((a, b) => b[1].amount - a[1].amount).map(([name, d]) => {
                        const maxAmt = Math.max(...Object.values(byMethod).map(v => v.amount), 1); return (
        ...
        {/* ═══ INTEGRASI PERTAMINA (Monica + MAP + BRIMOLA) ═══ */}
        {role && page === "integration" && (() => {
          const SYSTEMS = [
            {
          ...
          return (
            <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.5)", backdropFilter: "blur(6px)", zIndex: 50, display: "flex", justifyContent: "center", alignItems: "flex-start", padding: 20, overflowY: "auto", animation: "fadeIn .2s" }} onClick={() => setPage("dashboard")}>
              <div style={{ background: "var(--card)", borderRadius: 20, padding: 28, width: "100%", maxWidth: 980, border: "1px solid var(--border)", animation: "slideUp .3s", margin: "20px 0" }} onClick={e => e.stopPropagation()}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 24 }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 14 }}>
                ...
                <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(160px,1fr))", gap: 12, marginBottom: 24 }}>
                  {[{ l: "Sistem Terhubung", v: SYSTEMS.length, c: "#10b981", ic: "link" }, { l: "Data Synced", v: totalSynced, c: "#0ea5e9", ic: "check" }, { l: "Pending Sync", v: totalPending, c: totalPending > 0 ? "#f59e0b" : "#10b981", ic: "clock" }, { l: "Regulasi Aktif", v: "Ditjen Migas 2025", c: "#8b5cf6", ic: "layers" }].map((s, i) => (
                    <div key={i} style={{ background: "var(--card2)", borderRadius: 12, padding: 14 }}><div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 6 }}><div style={{ width: 22, height: 22, borderRadius: 6, background: `${s.c}15`, color: s.c, display: "flex", alignItems: "center", justifyContent: "center" }}><Ic name={s.ic} size={11} /></div><span style={{ fontSize: 9, fontWeight: 700, color: "var(--text-dim)", textTransform: "uppercase" }}>{s.l}</span></div><div className="mono" style={{ fontSize: 18, fontWeight: 800, color: s.c }}>{s.v}</div></div>
                  ))}
                </div>
